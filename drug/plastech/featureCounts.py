    ...
    def add_tag(self, bam, gtf):
        ...
        for read in samfile:
            ...
            if read.has_tag('XT'):
                ...
                gene_name = id_name[gene_id]['gene_name']
                gene_biotype = id_name[gene_id]['gene_biotype']
                read.set_tag(tag='GN', value=gene_name, value_type='Z')
                read.set_tag(tag='GX', value=gene_id, value_type='Z')
                read.set_tag(tag='GT', value=gene_biotype, value_type='Z')
            new_bam.write(read)
        new_bam.close()
...
def get_featureCounts_para(parser, optional=False):
    if optional:
        parser.add_argument(
            '--gtf_type',
            help='Specify feature type in GTF annotation. Default `exon`.',
            default='exon'
        )
